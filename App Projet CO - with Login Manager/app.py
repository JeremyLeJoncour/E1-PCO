...
def database():
    if "user" in session:
        ...
        if request.method == 'POST':
            ...
            list_of_dico = connect.plot_2(Results)
    ...
    if "user" in session:
        ...
        if request.method == 'POST':
            ...
            return render_template(
                ...
                data_curve = data_curve, data0 = data0, data1 = data1, sauvegarde = sauvegarde, user = user, date_valuate = date_valuate,
                hist_AUC = hist_AUC, hist_F1 = hist_F1, hist_MCC = hist_MCC, hist_Recall = hist_Recall, 
                hist_Precision = hist_Precision, hist_loss = hist_loss)
    else:
        return redirect(url_for("login"))
